...
static void cmd_help(void) {
    ...
    terminal_write("  mem       memory usage\n");
...
static void cmd_security(void) {
    ...
    terminal_setcolor(VGA_LIGHT_GREEN, VGA_BLACK);
    terminal_write("enabled (0x100000-0x3FFFFF)\n");
    terminal_setcolor(VGA_LIGHT_GREY, VGA_BLACK);
...
static void cmd_version(void) {
    ...
    terminal_write("  Paging  : PAE, identity mapped 0x00000000-0x003FFFFF\n");
    terminal_write("  NX      : 0x100000+\n");
    terminal_write("  Build   : NASM + GCC -m32 -fstack-protector-strong\n");
}

static void shell_execute(const char* cmd) {
    if      (streq(cmd, "help"))     cmd_help();
    else if (streq(cmd, "clear"))    cmd_clear();
    else if (streq(cmd, "mem"))      cmd_mem();
    else if (streq(cmd, "security")) cmd_security();
...
void shell_handle_char(char c) {
    if (c == '\n') {
        terminal_putchar('\n');
        cmd_buf[cmd_len] = '\0';
        if (cmd_len > 0) shell_execute(cmd_buf);
        cmd_len = 0;
        print_prompt();
    } else if (c == '\b') {
        if (cmd_len > 0) { cmd_len--; terminal_putchar('\b'); }
